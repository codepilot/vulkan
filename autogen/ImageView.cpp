...
namespace vulkan_autogen {
	Eternal<Function> ImageView::constructor;
	...
	ImageView::ImageView(const FunctionCallbackInfo<Value>& args) {
		...
		vkCreateImageView = reinterpret_cast<PFN_vkCreateImageView>(vkGetDeviceProcAddr(ObjectWrap::Unwrap<Device>(parent_device.Get(isolate))->vulkan_handle,"vkCreateImageView"));
		vkDestroyImageView = reinterpret_cast<PFN_vkDestroyImageView>(vkGetDeviceProcAddr(ObjectWrap::Unwrap<Device>(parent_device.Get(isolate))->vulkan_handle,"vkDestroyImageView"));
	
				VkImageViewCreateInfo pCreateInfo;
		memset(&pCreateInfo, 0, sizeof(VkImageViewCreateInfo));
		pCreateInfo.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;
		vkCreateImageView(ObjectWrap::Unwrap<Device>(parent_device.Get(isolate))->vulkan_handle, &pCreateInfo, nullptr, &vulkan_handle);
	...
	ImageView::~ImageView() {
		...
		vkDestroyImageView(ObjectWrap::Unwrap<Device>(parent_device.Get(isolate))->vulkan_handle, vulkan_handle, nullptr);

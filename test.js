...
function debugCB(flags, objectType, object, location, messageCode, pLayerPrefix, pMessage, pUserData) {
...
function test_vulkan_level_20() {
		...
		console.log('createDebugReportCallbackEXT');
		const debugCreateInfo = {
			flags: 0x1f,
			pfnCallback: debugCB,
			pUserData: {"Hello": "World"}
		};

		const debugReportCallbackEXT = inst.createDebugReportCallbackEXT(debugCreateInfo);
		console.dir(debugReportCallbackEXT, { showHidden: true, depth: null, colors: true });
		{
			...
			console.log('createCommandPool');
			const commandPool = device.createCommandPool();
			console.dir(commandPool, { showHidden: true, depth: null, colors: true });

			console.log('createBuffer');
			const buffer = device.createBuffer();
			console.dir(buffer, { showHidden: true, depth: null, colors: true });
